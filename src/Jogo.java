...
public class Jogo {
	 ...
	 * estratégias com bugs. Caso o numero de peças de um dos jogadores seja 0, é 
	 * garantida sua vitória e analisada seu tipo de vitória dependendo da ultima peça jogada.
	 * 
	...
	public void rodada() throws EstrategiaInvalidaException, JogadaInvalidaException {
		...
		if (jogador1.getNumPecas() == 0) {
			...
			if(jogador1.getUltimaJogada().getTipo() == TipoJogada.DIREITA_ESQUERDA) {
				if(jogador1.getUltimaJogada().getPeca().ehCarroca()) {
					this.tipoVitoria = "cruzada";
				} else {
					this.tipoVitoria = "laELo";
				}
			} else {
				if(jogador1.getUltimaJogada().getPeca().ehCarroca()) {
					this.tipoVitoria = "carroça";
				} else {
					this.tipoVitoria = "normal";
				}
			}
			return;
		}
		...
		if (jogador2.getNumPecas() == 0) {
			...
			if(jogador2.getUltimaJogada().getTipo() == TipoJogada.DIREITA_ESQUERDA) {
				if(jogador2.getUltimaJogada().getPeca().ehCarroca()) {
					this.tipoVitoria = "cruzada";
				} else {
					this.tipoVitoria = "laELo";
				}
		...
		// se ambos passaram, possuírem a mesma quantidade de peças e de soma no numero de peças, é empate
		if (jogadaJ1.getTipo() == TipoJogada.PASSA && jogadaJ2.getTipo() == TipoJogada.PASSA) {
			if(jogador1.getNumPecas() != jogador2.getNumPecas()) {
				...
				if(jogador1.getSomaMao() < jogador2.getSomaMao()) {
					this.finalizado = true;
					this.vencedor = this.jogador1.getNome();
				} else if (jogador1.getSomaMao() > jogador2.getSomaMao()){
					this.finalizado = true;
					this.vencedor = this.jogador2.getNome();
				} else {
					this.finalizado = true;
	...
	/**
	 * Joga o jogo do ponto atual até o seu fim. Definindo um histórico
	 * que armazena o vencedor e o tipo de vitória garantida por ele.
	 * @return Retorna o Historico do jogo completo
	 * 
	 * @throws EstrategiaInvalidaException Se a estratégia de um dos jogadores
	 *                                     decidir jogar uma peça que ele não
	 *                                     possui.
	...
	public HistoricoDeJogo jogaJogoCompleto() throws EstrategiaInvalidaException, JogadaInvalidaException {
		...
		if(this.isResultadoEmpate()) {
			...
			jogado.setTipoPontuacao(getTipoVitoria());
		}
...
//		System.out.println("==> final: " + (venceu == null ? "EMPATE" : venceu + " VENCEU") + "\n");
	...
	/**
	 * @return Tipo de vitória, ou null caso o jogo não esteja finalizado.
	 */
	public String getTipoVitoria() {
		return tipoVitoria;
	}
